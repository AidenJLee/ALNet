...
- (NSDictionary *)hasErrorForResponse:(NSHTTPURLResponse *)response withData:(NSData *)data
{
    ...
    if ([self.statusCodes containsIndex:(NSUInteger)res.statusCode]) {
        ...
        } else if ([mineType isEqualToString:@"text/html"]) {   // html이면 에러페이지라고 판단
...
- (id)objectForResponse:(NSURLResponse *)response data:(NSData *)data
{
    ...
    NSDictionary *errorInfo = [self hasErrorForResponse:(NSHTTPURLResponse *)response withData:data];
    if (!errorInfo) {
